...
void spRainfallGet(SPNoise* noise, 
	...
	double* outRainfall1)
{
	SPVec3 scaledNoiseLoc = spVec3Mul(noiseLoc, 51.0);
	...
	double riverAddition = (1.0 - pow(riverDistance, 0.2));
	...
	*outRainfall1 = (pregenRainfall1 + noiseValueS * (10.0 + pregenRainfall1 * 1.0)) * worldGenOptions.rainfallMultiplier + riverAddition;// + riverAddition * (4.0 + noiseValueW);
